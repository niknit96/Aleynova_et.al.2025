...
ggsave("../fig 6a. Shannon (Organ.material) (ITS).png", width=3, height=4, dpi=400)
```
## Shannon (Season)
```{r fig.width=5, fig.height=6}
...
t1$plot_alpha(measure = "Shannon") +
	scale_y_continuous(breaks = c(0,1,2,3,4,5,6), limits = c(2,6))
ggsave("../fig 6b. Shannon (Season) (ITS).png", width=3, height=4, dpi=400)
```
## Shannon (Plant)
```{r fig.width=5, fig.height=6}
...
t1$plot_alpha(measure = "Shannon") +
	scale_y_continuous(breaks = c(0,1,2,3,4,5,6), limits = c(2,6))
ggsave("../fig 6c. Shannon (Plant) (ITS).png", width=3, height=4, dpi=400)
```
...
ggsave("../fig 6d. Bray-curtis (Organ.material) (ITS).png", width=4, height=4, dpi=400)
```
...
ggsave("../fig 6e. Bray-curtis (Season) (ITS).png", width=4, height=4, dpi=400)
```
## PERMANOVA (Season)
```{r }
...
ggsave("../fig 6f. Bray-curtis (Plant) (ITS).png", width=4, height=4, dpi=400)
```
## PERMANOVA (Plant)
```{r }
...
ggsave("../fig 7a. Organ material bar plot (Class) (ITS).png", width=5, height=6, dpi=400)
```
...
t1$plot_venn()
ggsave("../fig 7d. Organ material venn diagramm (ASV) (ITS).png", width=5, height=5, dpi=400)
```
```{r}
datatable(t1$data_details, options = list(scrollX = TRUE, scrollY = TRUE))
write.table(t1$data_details, sep = "\t", quote = FALSE, file="Organ material venn diagramm (ASV) (ITS).txt")
```
...
geom_text(aes(x = Sample, y = Abundance, 
		...
		group = Taxonomy), 
		inherit.aes = FALSE, position = position_stack(vjust = 0.5), col = "black")
ggsave("../fig 7b. Season bar plot (Class) (ITS).png", width=5, height=6, dpi=400)
```
...
t1 <- trans_venn$new(ven, ratio = "seqratio")
t1$plot_venn()
ggsave("../fig 7e. Season venn diagramm (ASV) (ITS).png", width=5, height=5, dpi=400)
```
```{r}
datatable(t1$data_details, options = list(scrollX = TRUE, scrollY = TRUE))
...
geom_text(aes(x = Sample, y = Abundance, 
		...
		group = Taxonomy), 
		inherit.aes = FALSE, position = position_stack(vjust = 0.5), col = "black")
ggsave("../fig 7c. Plant bar plot (Phylum) (ITS).png", width=5, height=6, dpi=400)
```
...
t1 <- trans_venn$new(ven, ratio = "seqratio")
t1$plot_venn()
ggsave("../fig 7f. Plant venn diagramm (ASV) (ITS).png", width=5, height=5, dpi=400)
```
```{r}
datatable(t1$data_details, options = list(scrollX = TRUE, scrollY = TRUE))
write.table(t1$data_details, sep = "\t", quote = FALSE, file="Plant venn diagramm (ITS) (16S).txt")
```
...
p1
ggsave("../fig 8. core (ITS).png", width=8, height=5, dpi=400)
```
...
png("../fig 9. Heatmap Organ.material (ITS).png", width=7, height=6, units = "in", res=400)
htmp = psq %>%
...
png("Heatmap Season (ITS).png", width=7, height=6, units = "in", res=400)
htmp = psq %>%
	comp_heatmap(name = "Relative abundance", sample_names_show = T, numbers_use_counts = FALSE,
...
png("../fig S11. Heatmap Plant (ITS).png", width=7, height=6, units = "in", res=400)
htmp = psq %>%
	comp_heatmap(name = "Relative abundance", sample_names_show = T, numbers_use_counts = FALSE,
